...
const EnrollmentForm = () => {
  ...
  return (
    <section id="enroll" className="relative py-24 lg:py-32 overflow-hidden bg-white">
      ...
      <div className="absolute inset-0 pointer-events-none">
        ...
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full h-full opacity-[0.03]" 
          style={{ backgroundImage: `radial-gradient(#0075CF 1px, transparent 1px)`, backgroundSize: '40px 40px' }} 
        />
      </div>
      ...
      <div className="container-width px-4 relative z-10">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="text-center mb-16 md:mb-24">
            ...
            >
              <Zap className="w-3.5 h-3.5 fill-[#0075CF]/20" /> Accelerated Enrollment
            </motion.div>
            <motion.h2 
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
            ...
            >
              ...
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#0075CF] via-[#00aaff] to-[#FD5A1A]">Learning Journey Today</span>.
            </motion.h2>
            <motion.p 
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
            ...
            >
              Transform your aspirations into a career-defining reality. Join 2000+ graduates who have launched their futures through AOTMS.
            </motion.p>
          </div>
            ...
            <div className="lg:col-span-5 space-y-10 py-4">
              ...
              >
                {[
                  ...
                  >
                    <div className={`w-14 h-14 rounded-2xl ${item.color} flex items-center justify-center flex-shrink-0 shadow-sm transition-transform group-hover:scale-110 group-hover:rotate-3`}>
                      <item.icon className="w-7 h-7" />
                    </div>
                    <div>
                      <h4 className="text-xl font-bold text-slate-900 mb-1 tracking-tight">{item.title}</h4>
                      <p className="text-slate-500 font-medium leading-normal text-sm">{item.desc}</p>
                    </div>
                  </motion.div>
                ))}
              </motion.div>
              ...
              >
                 ...
                 <div className="relative z-10 flex items-center gap-6">
                    ...
                    <div>
                       <p className="text-2xl font-black tracking-tight mb-0.5">85% Success Rate</p>
                       <p className="text-xs font-bold text-white/50 uppercase tracking-widest">Average Graduate Salary Increase</p>
                    </div>
                 </div>
              </motion.div>
            </div>
                  ...
                  <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                      ...
                      <div className="space-y-3">
                        <motion.label 
                          animate={{ color: isFocused === 'name' ? '#0075CF' : '#94a3b8' }}
                          className="text-[10px] font-black uppercase tracking-[0.2em] ml-2"
                        >
                          Identity
                        </motion.label>
                        <div className="relative">
                          <User className={`absolute left-5 top-1/2 -translate-y-1/2 w-5 h-5 transition-all duration-300 ${isFocused === 'name' ? 'text-[#0075CF] scale-110' : 'text-slate-300'}`} />
                          <Input 
                            {...register("name")}
                          ...
                          <AnimatePresence>
                            {errors.name && (
                              <motion.p initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="text-red-500 text-[9px] mt-2 ml-2 font-black uppercase tracking-tighter italic">
                                {errors.name.message}
                              </motion.p>
                            )}
                          </AnimatePresence>
                      ...
                      <div className="space-y-3">
                        <motion.label 
                          animate={{ color: isFocused === 'email' ? '#0075CF' : '#94a3b8' }}
                          className="text-[10px] font-black uppercase tracking-[0.2em] ml-2"
                        >
                          Communication
                        </motion.label>
                        <div className="relative">
                          <Mail className={`absolute left-5 top-1/2 -translate-y-1/2 w-5 h-5 transition-all duration-300 ${isFocused === 'email' ? 'text-[#0075CF] scale-110' : 'text-slate-300'}`} />
                          <Input 
                            {...register("email")}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                       ...
                       <div className="space-y-3">
                        <motion.label 
                          animate={{ color: isFocused === 'phone' ? '#FD5A1A' : '#94a3b8' }}
                          className="text-[10px] font-black uppercase tracking-[0.2em] ml-2"
                        >
                          Contact Mobile
                        </motion.label>
                        <div className="relative">
                          <Phone className={`absolute left-5 top-1/2 -translate-y-1/2 w-5 h-5 transition-all duration-300 ${isFocused === 'phone' ? 'text-[#FD5A1A] scale-110' : 'text-slate-300'}`} />
                          <Input 
                            {...register("phone")}
                      ...
                      <div className="space-y-3">
                        <motion.label 
                          animate={{ color: isFocused === 'course' ? '#0075CF' : '#94a3b8' }}
                          className="text-[10px] font-black uppercase tracking-[0.2em] ml-2"
                        >
                          Career Path
                        </motion.label>
                        <div className="relative group">
                          <BookOpen className={`absolute left-5 top-1/2 -translate-y-1/2 w-5 h-5 z-10 transition-all duration-300 ${isFocused === 'course' ? 'text-[#0075CF] scale-110' : 'text-slate-300'}`} />
                          <Select 
                            onValueChange={(v) => {
                              setValue("course", v);
                              setIsFocused('course');
                          ...
                          >
                            ...
                            <SelectContent className="rounded-2xl border-slate-100 shadow-2xl p-2">
                              <SelectItem className="rounded-xl py-3 font-bold focus:bg-blue-50 focus:text-primary transition-colors" value="fullstack">Web Application Core (MERN)</SelectItem>
                              <SelectItem className="rounded-xl py-3 font-bold focus:bg-blue-50 focus:text-primary transition-colors" value="datascience">Data Intelligence & AI</SelectItem>
                              <SelectItem className="rounded-xl py-3 font-bold focus:bg-blue-50 focus:text-primary transition-colors" value="cloud">Cloud Architecture & AWS</SelectItem>
                              <SelectItem className="rounded-xl py-3 font-bold focus:bg-blue-50 focus:text-primary transition-colors" value="cyber">Advanced Cyber Resilience</SelectItem>
                            </SelectContent>
                          </Select>
                          <AnimatePresence>
                            {errors.course && (
                    ...
                    <div className="flex items-center justify-center gap-8 pt-4">
                       <div className="flex items-center gap-2">
                          ...
                          <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Free Demo Class</span>
                       </div>
                       <div className="flex items-center gap-2">
                          <CheckCircle2 className="w-3.5 h-3.5 text-emerald-500" />
                          <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Expert Consultation</span>
                       </div>
                    </div>
